"""
デジタル寺子屋 第3回  機械学習にトライ - 02 ボストンの住宅価格の予測(1) 単回帰分析
...
　Pythonを使ったデータ分析による機械学習の基礎を学びます。
 ...
 単回帰とは、以下の説明変数のうち1つのみを使って目的変数を精度よく予測しようとする
 分析手法です。
...
print("[08] 学習用データの欠測値の個数（X,Y）＝", X_train.isnull().sum(), y_train.isnull().sum())
print("[09] 評価用データの欠測値の個数（X,Y）＝", X_test.isnull().sum(), y_test.isnull().sum())

# 説明変数・目的変数間の相関関係の確認
#  * 相関係数（-1: 真逆の関係、 0: 相関なし、 1: 正比例の関係）による相関分析
print("[10] 相関係数＝\n", original_data[[MOKUTEKI, SETSUMEI]].corr())
#  * 散布図による説明変数Xと目標変数Yの相関分析
plt.scatter(X_train, y_train, c="Blue") # X_trainとY_trainの散布を青ドットで描画
plt.xlabel("X_train")   # グラフのX軸の文字列
plt.ylabel("y_train")   # グラフのY軸の文字列
plt.show()              # グラフの表示
...
# 評価用データと予測結果を対比してグラフ上に表示
y_predict = model.predict(X_test)
y_test = y_test.values.tolist()
plt.scatter(y_predict, y_predict-y_test, color = 'blue')  # 残差をプロット
plt.hlines(y = 0, xmin = -10, xmax = 50, color = 'black') # x軸に沿った直線をプロット
plt.xlabel('y_predict')                                   # x軸のラベル
plt.ylabel('error(y_predict - y_test)')                   # y軸のラベル
plt.grid()                                                # グリッド線を表示
plt.show()                                                # 図の表示
